...
import { Compass, Camera, User, Mail, Phone, MapPin, Globe, FileText, ArrowRight, KeyRound, CheckCircle2, RefreshCw } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setSuccessMsg('Verification code sent to your email! (Demo mode active)');
      setStep('otp');
    } else {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (otpToken === '123456' || process.env.NEXT_PUBLIC_SUPABASE_URL?.includes('placeholder')) {
        ...
        setErrorMsg(error.message || 'Invalid code. Use 123456 in demo mode.');
      }
    } else {
      router.push('/dashboard');
    }
        ...
        <div className="bg-white rounded-3xl border border-slate-200/90 shadow-soft-lg p-6 sm:p-10 space-y-6 relative overflow-hidden">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="font-heading font-extrabold text-2xl text-slate-900">
              {step === 'details' ? 'Create Your Traveler Account' : 'Verify Email OTP Code'}
            </h1>
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-6">
              ...
              <div className="flex flex-col items-center justify-center space-y-2 bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
                ...
                <div className="flex items-center gap-2 pt-1">
                  <span className="text-[11px] text-slate-500 font-medium">Select avatar:</span>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Travel Style & Bio</label>
                <textarea
                  rows={3}
                  value={bio}
                  onChange={(e) => setBio(e.target.value)}
                  placeholder="Tell us about your travel style (slow travel, hiking, culinary, luxury)..."
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4 max-w-md mx-auto">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-1">
                  (In demo mode, type <span className="font-mono font-bold text-ocean-700">123456</span> to complete registration)
                </p>
              </div>
